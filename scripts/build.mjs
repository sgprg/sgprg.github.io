...
const tags = values => `<ul class="tags">${values.map(t=>`<li>${e(t)}</li>`).join('')}</ul>`;
const label = (n,title) => `<div class="section-label"><span>${n}</span><p>${title}</p></div>`;
const external = (url, text, cls = '') => `<a class="${cls}" href="${e(url)}" target="_blank" rel="noopener noreferrer">${e(text)} ${arrow}</a>`;

export function renderProfile(input) {
  const p = validateProfile(input);
  const schema = JSON.stringify({'@context':'https://schema.org','@type':'Person',name:p.name,url:p.siteUrl,description:p.intro,jobTitle:p.current,sameAs:[p.linkedin,p.github],email:`mailto:${p.email}`,knowsAbout:p.expertise.flatMap(x=>x.skills)}).replace(/</g,'\\u003c');
  return `<!doctype html>
...
<nav aria-label="Main navigation"><a href="#about">About</a><a href="#experience">Experience</a><a href="#work">Work</a><a href="#writing">Writing</a></nav><a class="header-contact" href="#contact">Let’s talk ${arrow}</a></header>
...
<div class="hero-actions"><a class="button primary" href="#work">Explore my work <span aria-hidden="true">↓</span></a><a class="text-link" href="./cv/">View & print CV ${arrow}</a></div>
...
<section id="experience" class="section page-width" aria-labelledby="experience-title">${label('02','Experience')}<div class="section-body"><div class="section-heading"><h2 id="experience-title">A career across the stack.</h2><a class="text-link" href="./cv/">Full CV ${arrow}</a></div><div class="timeline">${p.experience.map((x,i)=>`<article class="role-entry"><div class="role-date"><span>${e(x.period)}</span>${i===0?'<span class="present-label">Current</span>':''}</div><div><h3>${e(x.company)}</h3><p class="job-title">${e(x.role)}</p><p>${e(x.summary)}</p><details><summary>Scope & focus <span aria-hidden="true">+</span></summary><ul>${x.highlights.map(h=>`<li>${e(h)}</li>`).join('')}</ul></details>${tags(x.tags)}</div></article>`).join('')}</div><p class="career-note">${e(p.additionalExperience)}</p><div class="credentials"><div><h3>Education & credentials</h3><p>${e(p.education)}</p><ul>${p.credentials.map(c=>`<li>${e(c)}</li>`).join('')}</ul></div><div><h3>Languages</h3><p>${e(p.languages)}</p></div></div></div></section>
<section id="work" class="work-section" aria-labelledby="work-title"><div class="section page-width">${label('03','Selected work')}<div class="section-body"><div class="section-heading"><h2 id="work-title">Ideas, put to work.</h2>${external(p.github,'GitHub','text-link')}</div><p class="section-lead">Independent projects, practical experiments and a few things built simply because they should exist.</p><div class="projects-grid">${p.projects.map((x,i)=>`<article class="project-card"><div class="project-top"><span class="project-index">${String(i+1).padStart(2,'0')} /</span><span class="project-status">${e(x.status)}</span></div><h3>${e(x.name)}</h3><p class="project-subtitle">${e(x.subtitle)}</p><p>${e(x.description)}</p><p class="project-focus">${e(x.focus)}</p>${x.url?external(x.url,x.linkLabel,'text-link'):''}</article>`).join('')}</div><p class="projects-note">${e(p.projectsNote)}</p></div></div></section>
...
<section id="contact" class="contact-section page-width" aria-labelledby="contact-title"><div><p class="eyebrow">Have a useful problem?</p><h2 id="contact-title">${lines(p.contactTitle)}</h2><p>${e(p.contactText)}</p></div><div class="contact-links"><a class="email-link" href="mailto:${e(p.email)}">${e(p.email)} ${arrow}</a><div>${external(p.linkedin,'LinkedIn','text-link')}${external(p.github,'GitHub','text-link')}</div><button class="copy-email" type="button" data-email="${e(p.email)}">Copy email address</button><span id="copy-status" class="sr-only" role="status"></span></div></section>
...
export function renderCv(p) {
  validateProfile(p);
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(p.name)} — CV</title><meta name="robots" content="noindex"><link rel="icon" href="../assets/favicon.svg"><link rel="stylesheet" href="../cv.css"><script src="../cv.js" defer></script></head><body><div class="cv-toolbar"><a href="../">← Back to biography</a><button id="print-cv">Print / Save as PDF</button></div><main><header><h1>${e(p.name)}</h1><p class="role">${e(p.role)}</p><p>${e(p.location)} · <a href="mailto:${e(p.email)}">${e(p.email)}</a> · <a href="${e(p.linkedin)}">LinkedIn / sergeyrh</a> · <a href="${e(p.github)}">GitHub / sgprg</a></p></header><section><h2>Profile</h2><p>${e(p.intro)}</p></section><section><h2>Experience</h2>${p.experience.map(x=>`<article><div class="job-heading"><h3>${e(x.company)}</h3><span>${e(x.period)}</span></div><p class="job-role">${e(x.role)}</p><p>${e(x.summary)}</p><ul>${x.highlights.map(h=>`<li>${e(h)}</li>`).join('')}</ul></article>`).join('')}<p>${e(p.additionalExperience)}</p></section><section><h2>Selected work</h2>${p.projects.map(x=>`<article><h3>${e(x.name)} <span class="status">${e(x.status)}</span></h3><p>${e(x.description)}${x.url?` <a href="${e(x.url)}">${e(x.url)}</a>`:''}</p></article>`).join('')}</section><section><h2>Expertise</h2>${p.expertise.map(x=>`<p><strong>${e(x.title)}:</strong> ${e(x.skills.join(', '))}.</p>`).join('')}</section><section><h2>Education & credentials</h2><p>${e(p.education)}</p><p>${e(p.credentials.join(' · '))}</p><p><strong>Languages:</strong> ${e(p.languages)}</p></section></main></body></html>`;
}

export async function build({ directory = path.join(root,'dist'), profile } = {}) {
  const p = profile ?? JSON.parse(await readFile(path.join(root,'content/profile.json'),'utf8'));
  const html = renderProfile(p), cv = renderCv(p);
  ...
  await writeFile(path.join(directory,'sitemap.xml'),`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>${e(p.siteUrl)}</loc><lastmod>${e(p.updated)}</lastmod></url></urlset>`);
  return p;
}

if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) { await build(); console.log('Built biography and print CV in dist/.'); }
